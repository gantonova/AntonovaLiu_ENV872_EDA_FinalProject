...
fire.p <- fire %>%
  select(Date, REPORT_AC,GIS_ACRES) %>%
...
```{r fire time series}
library(Kendall)
fire.ts <- ts(fire.sum$summonth, start = c(1992,2,1), frequency = 12)
#decompose
fire.Decomposed <- stl(fire.ts, s.window = "periodic")
plot(fire.ts.year, main = "Fire Extent Decomposed")
#Mann Kendall test
fire.trend1 <- Kendall::SeasonalMannKendall(fire.ts)
summary(fire.trend1)

#year time series
fire.ts.year <- ts(fire.yearsum$summonth, start = c(1992,2,1), frequency = 1)
#decompose
fire.year.Decomposed <- stl(fire.ts.year, s.window = "periodic")
plot(fire.Decomposed, main = "Fire Extent Decomposed")
#Mann Kendall test
fire.trend1.year <- Kendall::SeasonalMannKendall(fire.ts.year)
summary(fire.trend1.year)
...
mod.glm <- glm(data = yosefire, formula = Visits ~ as.factor(Month)+ Year + Sumfire + Firecounts, family = poisson())
summary(mod.glm)
plot(mod.glm)

mod.lmfactorint <- lm(data=yosefire, Visits ~ as.factor(Month)*Firecounts)
summary(mod.lmfactorint)
par(mfrow=c(2,2))
